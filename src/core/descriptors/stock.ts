 ...
 * anywhere in TypeScript: a profile's `objects` reference these by name (`"cb"`) or ship
 * their own inline descriptor — there is no cross-profile inheritance (a derived chart is a
 * duplicated, self-contained file). The only thing that can't live in JSON — the custom
 * RENDERING — is a NAMED extension (`jet-barbs`, `front-symbols`), registered in `registry.ts`.
 */
...
import stockGlyphs from "./stock-glyphs.json" with { type: "json" };
...
/** The stock glyphs the lib needs to compile its built-in registry — GENERATED by
 *  `npm run build:stock` from the glyphs `wafs.json` (the built-in default profile)
 *  references, no more (TEMSI-only button icons and the WMO map symbols stay out of the
 *  lib; they're embedded into a profile only when referenced). */
export const STOCK_GLYPHS: Record<string, string> = stockGlyphs as Record<string, string>;
