...
| `test/placement.test.ts` | `map/placement` (pure) | Call-out anti-collision **cartouche bound**: boxes clamped inside the chart frame, leader tips protected (no box covers another feature's tip), obstacle **padding** (no flush contact), and frame/pad-free = legacy behaviour. |
